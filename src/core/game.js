...
export function defaultState(){
  return {
    ...
    // 生鮮品は取得時刻と寿命を持つ。container は bag / storage。
    perishables:[],
    // 将来の調合・発酵・熟成を同じステップ時計に接続するための汎用タイマー。
    timedProcesses:[],
...
export function derived(state){
  const p=state.player,w=items[p.equipment.weapon],a=items[p.equipment.armor],mul=1-fatiguePenalty(state);
  const raw={maxHp:p.baseMaxHp+levelHp(p.level)+(w?.hp||0)+(a?.hp||0),maxMp:p.baseMaxMp+levelMp(p.level)+(w?.mp||0)+(a?.mp||0),atk:p.baseAtk+levelAtk(p.level)+(w?.atk||0)+(a?.atk||0),def:p.baseDef+levelDef(p.level)+(w?.def||0)+(a?.def||0)};
...
export function normalize(state){
  const base=defaultState(),old=state||{},s={...base,...old,version:4};
  s.player={...base.player,...(old.player||{})};s.player.equipment={...base.player.equipment,...(old.player?.equipment||{})};
  s.gold=Number.isFinite(old.gold)?old.gold:(old.player?.gold||base.gold);
  s.inventory={...base.inventory,...(old.inventory||{})};
  s.consumables={...base.consumables,...(old.consumables||{})};s.ownedItems={...base.ownedItems,...(old.ownedItems||{})};
  ...
  s.backpack=backpacks[old.backpack]?old.backpack:'cheap';s.settings={...base.settings,...(old.settings||{})};
  s.timedProcesses=Array.isArray(old.timedProcesses)?old.timedProcesses.map(x=>({...x})) : [];
  migratePerishables(s,old);
  if(s.run){
    s.run={cargo:{},pendingExp:0,harvested:[],visited:['town'],lastEvent:null,moves:0,effects:{encounterMod:0,moves:0},...s.run};delete s.run.freshHerbs;
    for(const [id,m] of Object.entries(materials)){const n=Number(s.run.cargo?.[id]||0);if(m.shelfLife&&n>0){addPerishableBatchRaw(s,id,n,'bag',s.calendar.totalSteps+m.shelfLife);delete s.run.cargo[id];}}
  }
...
function expirePerishables(state){
  const now=state.calendar.totalSteps||0,expired=[];const keep=[];
  for(const b of state.perishables||[]){if(b.expiresAt<=now)expired.push(b);else keep.push(b);}state.perishables=keep;
  return groupedCounts(expired);
}
function matureProcesses(state){const now=state.calendar.totalSteps||0,matured=[];for(const p of state.timedProcesses||[]){if(!p.ready&&p.readyAt<=now){p.ready=true;matured.push(p.id);}}return matured;}
...
export function advanceTime(state,steps,opts={}){
  ...
  const expired=expirePerishables(state),matured=matureProcesses(state),after=phaseInfo(state),d=derived(state);state.player.hp=Math.min(state.player.hp,d.maxHp);state.player.mp=Math.min(state.player.mp,d.maxMp);
...
function timeMessages(t){const out=[];if(t.changed)out.push(`${t.after.icon} ${t.day}日目 ${t.after.name}になった。`);for(const [id,n] of Object.entries(t.expired||{}))out.push(`${materials[id]?.name||id}×${n}が傷んだ。`);if(t.fatigueAfter>t.fatigueBefore)out.push(`徹夜疲労が進行。全能力 -${t.fatigueAfter*10}%。`);for(const id of t.matured||[])out.push(`${id} の熟成が完了した。`);return out;}
...
export function usedCapacity(state){
  ...
  if(state.run){for(const [id,n] of Object.entries(state.run.cargo||{}))used+=(materials[id]?.bulk||1)*n;used+=sum((state.perishables||[]).filter(x=>x.container==='bag'),x=>(materials[x.id]?.bulk||1)*x.count);}
  return used;
}
...
function addPerishable(state,id,count=1,container='bag'){
  const m=materials[id];if(!m?.shelfLife)return 0;const bulk=m.bulk||1;
  const added=container==='bag'?Math.min(count,Math.floor(freeCapacity(state)/bulk)):count;
...
export function startExpedition(state){
  ...
  state.run={location:'town',cargo:{},pendingExp:0,harvested:[],visited:['town'],lastEvent:null,moves:0,effects:{encounterMod:0,moves:0},startedAtStep:state.calendar.totalSteps};state.battle=null;
...
function applyEvent(state,event){const ef=event.effect||{};let suffix='';if(ef.herb){const n=addHerb(state,ef.herb);suffix=n?` 薬草+${n}`:' バッグがいっぱいで薬草は置いてきた。';}
  ...
  if(ef.steps){const t=advanceTime(state,ef.steps);suffix+=` ${ef.steps}ステップ経過。`;const msgs=timeMessages(t);if(msgs.length)suffix+=` ${msgs.join(' ')}`;}
...
export function travelTo(state,target){
  ...
  const timeNote=timeMessages(time).join(' ');
...
export function command(state,type){
  const b=state.battle;if(!b||b.over)return{ok:false};const e=enemies[b.enemyId],st=derived(state);b.guarding=false;let valid=true;
  ...
  else if(type==='skill'){if(state.player.mp<3){pushBattle(b,'MPが たりない！');valid=false;}else{state.player.mp-=3;const dmg=Math.max(2,Math.floor(st.atk*1.7)+rand(-2,4)-Math.floor(b.enemyDef*.35));b.enemyHp=Math.max(0,b.enemyHp-dmg);pushBattle(b,`火炎斬り！ ${dmg} ダメージ！`);}}
  else if(type==='heal'){if(state.player.mp<4){pushBattle(b,'MPが たりない！');valid=false;}else{state.player.mp-=4;const before=state.player.hp;state.player.hp=Math.min(st.maxHp,state.player.hp+18+state.player.level*4+rand(0,5));pushBattle(b,`ホイミ！ HPが ${state.player.hp-before} 回復した。`);}}
  else if(type==='defend'){b.guarding=true;pushBattle(b,'身を守っている。');}
  else if(type==='herb'){if(perishableCount(state,'fresh_herb','bag')<=0){pushBattle(b,'新鮮な薬草を持っていない。');valid=false;}else{removePerishable(state,'fresh_herb',1,'bag');const before=state.player.hp;state.player.hp=Math.min(st.maxHp,state.player.hp+24);pushBattle(b,`薬草を使った。HPが ${state.player.hp-before} 回復した。`);}}
  else if(type==='potion'){if((state.consumables.potion||0)<=0){pushBattle(b,'ポーションを持っていない。');valid=false;}else{state.consumables.potion--;const before=state.player.hp;state.player.hp=Math.min(st.maxHp,state.player.hp+45);pushBattle(b,`ポーションを使った。HPが ${state.player.hp-before} 回復した。`);}}
  else valid=false;
  if(!valid)return{ok:false};
  if(b.enemyHp<=0)victory(state);else enemyTurn(state);
  finishBattleTurn(state,b);return{ok:true};
}
...
export function defeatReturn(state){if(!state.run)return{ok:false};const lost={cargo:{...state.run.cargo},perishables:clearBagPerishables(state),exp:state.run.pendingExp};state.run=null;state.battle=null;state.player.hp=1;state.log.unshift('力尽き、荷物を失って村まで運ばれた。');return{ok:true,lost};}
...
export function camp(state){
  ...
  if(cs.safety==='semi'&&Math.random()<.35){const interrupted=Math.max(10,Math.ceil(steps/2)),t=advanceTime(state,interrupted,{awake:false}),rec=recoverCamp(state,.20),zone=zones[cs.node.zone],pool=zone?.pools?.[phaseInfo(state).key]||zone?.pools?.day||['slime'];beginEncounter(state,pick(pool),'camp_raid');state.run.lastEvent=`野営中に襲撃！ ${interrupted}ステップ経過。`;return{ok:true,raided:true,battle:true,steps:interrupted,msg:`うとうとしたところを襲われた！ HP+${rec.hp} / MP+${rec.mp}。${timeMessages(t).join(' ')}`};}
  const t=advanceTime(state,steps,{resting:true}),rec=recoverCamp(state,.70);state.run.lastEvent=`キャンプで${steps}ステップ休んだ。`;return{ok:true,raided:false,steps,msg:`キャンプ成功。HP+${rec.hp} / MP+${rec.mp}。疲労も解消した。${timeMessages(t).join(' ')}`};
}
...
export function sellMaterial(state,id,count=1){if(state.run)return{ok:false,msg:'売却は村にいる時だけ。'};const m=materials[id],have=materialCount(state,id),n=count==='all'?have:Math.min(have,Math.max(1,count));if(!m||n<=0)return{ok:false,msg:'売れる素材がない。'};let remaining=n;const fresh=Math.min(remaining,perishableCount(state,id,'storage'));if(fresh){removePerishable(state,id,fresh,'storage');remaining-=fresh;}if(remaining>0)state.inventory[id]=Math.max(0,(state.inventory[id]||0)-remaining);const gain=m.value*n;state.gold+=gain;return{ok:true,msg:`${m.name}×${n}を ${gain}G で売った。`,gain};}
export function sellAll(state){if(state.run)return{ok:false,msg:'売却は村にいる時だけ。'};let gain=0;for(const id of Object.keys(materials)){const n=materialCount(state,id);if(n>0){gain+=materials[id].value*n;state.inventory[id]=0;removePerishable(state,id,n,'storage');}}state.gold+=gain;return{ok:true,msg:`素材をまとめて ${gain}G で売った。`,gain};}

export function canCraft(state,r){return Object.entries(r.cost).every(([id,n])=>materialCount(state,id)>=n);}
export function craft(state,recipeId){if(state.run)return{ok:false,msg:'鍛冶は村で。'};const r=recipes.find(x=>x.id===recipeId);if(!r||!canCraft(state,r))return{ok:false,msg:'素材が足りない。'};for(const [id,n] of Object.entries(r.cost)){let left=n,per=Math.min(left,perishableCount(state,id,'storage'));if(per){removePerishable(state,id,per,'storage');left-=per;}if(left)state.inventory[id]-=left;}state.ownedItems[r.item]=(state.ownedItems[r.item]||0)+1;return{ok:true,msg:`${items[r.item].name} 完成！`};}
