...
export function defaultState(){
  ...
  return{
  version:14,player:hero,characters:{hero,boris},party:['hero'],
...
export function characterById(state,id='hero'){return state.characters?.[id]||((id==='hero'||!id)?state.player:null);}
...
export function derived(state){return derivedCharacter(state,state.player||'hero');}
...
export function normalize(state){
  const base=defaultState(),old=state||{},s={...base,...old,version:14};
  ...
  const oldHero=old.characters?.hero||old.player||{};
  const hero={...base.characters.hero,...oldHero,stats:{...base.characters.hero.stats,...legacyStats({...old,player:oldHero})},equipment:{...base.characters.hero.equipment,...(oldHero.equipment||{})},appPoints:Math.max(0,Number(oldHero.appPoints)||0),tactic:oldHero.tactic||'manual',recruited:true};
  const oldBoris=old.characters?.boris||{};
  const boris={...base.characters.boris,...oldBoris,stats:{...base.characters.boris.stats,...(oldBoris.stats||{})},equipment:{...base.characters.boris.equipment,...(oldBoris.equipment||{})},appPoints:Math.max(0,Number(oldBoris.appPoints)||0),tactic:TACTICS[oldBoris.tactic]?oldBoris.tactic:'gungun'};
  s.characters={hero,boris};s.player=hero;
  s.party=Array.isArray(old.party)?old.party.filter(id=>s.characters[id]&&s.characters[id].recruited!==false):['hero'];if(!s.party.includes('hero'))s.party.unshift('hero');s.party=[...new Set(s.party)].slice(0,4);
  ...
  s.nextGearId=Math.max(1,Number(old.nextGearId)||1);migrateGear(s,old);s.player.equipment=mapLegacyEquipment(s,old);s.characters.hero=s.player;
  ...
  for(const c of Object.values(s.characters)){const d=derivedCharacter(s,c);const source=old.characters?.[c.id]||(c.id==='hero'?old.player:null);c.hp=clamp(Number.isFinite(source?.hp)?source.hp:(Number.isFinite(c.hp)?c.hp:d.maxHp),0,d.maxHp);c.mp=clamp(Number.isFinite(source?.mp)?source.mp:(Number.isFinite(c.mp)?c.mp:d.maxMp),0,d.maxMp);}
  s.player=s.characters.hero;return s;
}
...
export function advanceTime(state,steps,opts={}){steps=Math.max(0,Math.floor(steps||0));const before=phaseInfo(state),beforeDay=state.calendar.day,oldFatigue=state.condition.fatigueStacks||0,newTotal=(state.calendar.totalSteps||0)+steps,expired=ageStacks(state,newTotal);state.calendar.totalSteps=newTotal;state.calendar.day=Math.floor(newTotal/DAY_STEPS)+1;state.calendar.stepOfDay=newTotal%DAY_STEPS;if(opts.resting){state.condition.awakeSteps=0;state.condition.fatigueStacks=0;}else if(opts.awake!==false){state.condition.awakeSteps=(state.condition.awakeSteps||0)+steps;state.condition.fatigueStacks=fatigueStacksForAwake(state.condition.awakeSteps);}const matured=matureProcesses(state),after=phaseInfo(state);for(const c of Object.values(state.characters||{hero:state.player})){const d=derivedCharacter(state,c);c.hp=Math.min(c.hp,d.maxHp);c.mp=Math.min(c.mp,d.maxMp);}return{before,after,changed:before.key!==after.key||beforeDay!==state.calendar.day,day:state.calendar.day,steps,expired:groupedExpired(expired),matured,fatigueBefore:oldFatigue,fatigueAfter:state.condition.fatigueStacks||0};}
...
export function useFieldItem(state,idOrStack){if(!state.run||state.battle)return{ok:false,msg:'探索中に使う道具ではない。'};let s=state.itemStacks.find(x=>x.stackId===idOrStack&&x.container==='bag');if(!s)s=stackList(state,'bag',idOrStack)[0];if(!s)return{ok:false,msg:'その道具を持っていない。'};if(s.id==='rura_potion')return useRura(state,s.stackId);const r=healWithStack(state,s.stackId);if(!r.ok)return r;const t=advanceTime(state,2),def=allDefs(r.stack.id),encounter=maybeContextEncounter(state,.92),notes=timeMessages(t);if(encounter)notes.push('道具を使っている間に魔物が近づいてきた！');return{ok:true,battle:encounter,steps:2,msg:`${def.name}${qualityLabel(r.stack.quality)}を使った。HP +${r.heal} / 2step経過。 ${notes.join(' ')}`};}
