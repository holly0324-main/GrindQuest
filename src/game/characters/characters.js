...
export function addExpByCharacter(state,amounts={},source='経験'){const members=partyMembers(state),byCharacter={},ups=[],rows=[];let leadLevels=0;for(const c of members){const amount=Math.max(0,Math.floor(Number(amounts?.[c.id])||0));byCharacter[c.id]=amount;rows.push({id:c.id,name:c.name,amount});const lv=amount?addCharacterExp(state,c,amount):0;if(c.id==='hero')leadLevels=lv;if(lv)ups.push(`${c.name} Lv.${c.level}`);}const leader=members[0],recorded=leader?byCharacter[leader.id]||0:0;if(recorded)recordExpGain(state,recorded);const unique=[...new Set(rows.map(x=>x.amount))],rewardText=unique.length===1?`+${unique[0]||0}`:rows.map(x=>`${x.name}+${x.amount}`).join(' / ');return{amount:recorded,byCharacter,levels:leadLevels,msg:`${source} EXP ${rewardText}${ups.length?` / ${ups.join('・')}！`:''}`};}
