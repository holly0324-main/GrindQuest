...
export function forgeRecipeVisible(state,r){const p=forgeRecipeProduct(r);if(!p||p.rank<1)return false;if(r.requiresUnlock&&!state?.unlocks?.recipes?.[r.id])return false;return Object.keys(r?.cost||{}).every(id=>isItemDiscovered(state,id));}
