...
assert.equal(m.version,15);assert.equal(m.characters.hero.level,7);assert.equal(m.characters.boris.level,5);assert.deepEqual(m.party,['hero','boris']);assert.equal(m.gold,432);assert.equal('player' in m,false);
...
assert.equal(ancient.version,15);assert.equal(ancient.characters.hero.level,1);assert.equal(ancient.gold,80);assert.equal(ancient.settings.vibrate,false);
